...
let AuthService = class AuthService {
    constructor(userRepository, responseServices, jwtService) {
    ...
    async login(loginUserDto) {
        ...
        if (!user)
            throw new common_1.UnauthorizedException("No se encontro este usuario");
        if (!bcrypt.compareSync(contrasena, user.contrasena))
            throw new common_1.UnauthorizedException("Las credenciales no coinciden");
        delete user.contrasena;
        return this.responseServices.responseSucces(200, 'Usuario ingresado correctamente', Object.assign(Object.assign({}, user), { token: this.getJwtoken({ id: user.id }) }));
    }
    getJwtoken(payload) {
